...
object DeploymentConfigData extends FromModelFactoryWithSchema[DeploymentConfigData]
{
	...
	override lazy val schema = 
		ModelDeclaration(Vector(PropertyDeclaration("projectId", IntType, Single("project_id")), 
			...
			isOptional = true), PropertyDeclaration("usesBuildDirectories", BooleanType, 
			Single("uses_build_directories"), true), PropertyDeclaration("fileDeletionEnabled", BooleanType, 
			Single("file_deletion_enabled"), true)))
	...
	extends DeploymentConfigFactory[DeploymentConfigData] with ModelConvertible
{
	...
	override def toModel = 
		Model(Vector("projectId" -> projectId, "outputDirectory" -> outputDirectory.toJson, 
