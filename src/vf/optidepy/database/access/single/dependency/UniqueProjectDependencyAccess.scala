...
object UniqueProjectDependencyAccess extends ViewFactory[UniqueProjectDependencyAccess]
{
	// IMPLEMENTED	--------------------
	...
	/**
	  ...
	  */
	override def apply(condition: Condition): UniqueProjectDependencyAccess = 
		_UniqueProjectDependencyAccess(Some(condition))
	
	
	// NESTED	--------------------
	
	private case class _UniqueProjectDependencyAccess(override val accessCondition: Option[Condition]) 
		extends UniqueProjectDependencyAccess
}

/**
  * A common trait for access points that return distinct project dependencies
  * @author Mikko Hilpinen
  * @since 09.08.2024, v1.2
  */
trait UniqueProjectDependencyAccess 
	extends UniqueDependencyAccessLike[ProjectDependency, UniqueProjectDependencyAccess] 
		with SingleRowModelAccess[ProjectDependency] with NullDeprecatableView[UniqueProjectDependencyAccess]
{
	// COMPUTED	--------------------
	...
	override def apply(condition: Condition): UniqueProjectDependencyAccess = 
		UniqueProjectDependencyAccess(condition)
